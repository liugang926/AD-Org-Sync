(() => {
  ...
  const conditionalScopeSections = Array.from(document.querySelectorAll("[data-scope-for]"));
  const updateScopeSections = () => {
    if (!(scopeSelect instanceof HTMLSelectElement)) return;
    conditionalScopeSections.forEach((section) => {
      const allowed = String(section.dataset.scopeFor || "").split(/\s+/).filter(Boolean);
      const visible = allowed.includes(scopeSelect.value);
      section.hidden = !visible;
      section.querySelectorAll("input, select, textarea, button").forEach((control) => {
        if (!(control instanceof HTMLElement)) return;
        if (visible) {
          if (control.dataset.scopeDisabled === "true") control.removeAttribute("disabled");
          delete control.dataset.scopeDisabled;
        } else if (!control.hasAttribute("disabled")) {
          control.setAttribute("disabled", "");
          control.dataset.scopeDisabled = "true";
        }
      });
    });
  };
  scopeSelect?.addEventListener("change", updateScopeSections);
  updateScopeSections();
  ...
  document.querySelector("[data-select-create-candidates]")?.addEventListener("click", () => {
    ...
    if (scopeSelect instanceof HTMLSelectElement) {
      scopeSelect.value = "selected_users";
      updateScopeSections();
    }
    updateStatus();
  });
