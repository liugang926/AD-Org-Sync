...
class DingTalkSourceProvider(SourceDirectoryProvider):
    ...
    def verify_employee_identity(self, request: Any) -> dict[str, Any] | None:
        ...
        if not callable(get_oauth_user_info):
            raise NotImplementedError("DingTalk API client does not support SSPR employee verification")
        payload = dict(get_oauth_user_info(oauth_code) or {})
        source_user_id = _extract_dingtalk_user_id(payload)
        ...
        return {
            ...
            "display_name": str(payload.get("name") or payload.get("nick") or source_user_id),
            "raw_claims": payload,
        }
...
def _extract_dingtalk_user_id(payload: dict[str, Any]) -> str:
    for key in ("userid", "userId", "staffid", "staffId", "unionid", "unionId", "openId", "open_id"):
