...
def require_valid_role(role: str | None) -> str:
    candidate = (role or "").strip().lower()
    if candidate not in WEB_ADMIN_ROLES:
        supported = ", ".join(WEB_ADMIN_ROLES)
        raise ValueError(f"unsupported administrator role: {role!r}; choose one of: {supported}")
    return candidate
