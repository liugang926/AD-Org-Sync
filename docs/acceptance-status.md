...
- 该单人预览已执行成功，新增一条本地有效绑定，指向预览确认的 AD objectGUID；执行前后的 AD 账号状态一致，未改动账号属性、密码或 OU。这提供了 AC-01 的真实唯一工号关联证据，但不等同于创建、移动、禁用或密码重置验收。[单人执行核验](https://github.com/liugang926/AD-Org-Sync/actions/runs/35845848332)
...
- 使用已验证的测试 OU 和专用测试对象完成真实 AD 写入，包括创建、属性、移动、禁用及本人密码重置；现有 `T0001919` 的本地绑定不能替代这些验收，禁止拿普通员工账号试验。
