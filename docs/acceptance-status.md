...
本文件记录证据边界，不作为自动宣布验收通过的依据。2026-09-23：旧平台已退役，Django 版本 `c53492163f73206ac1a4c5e3b0603be64986f05d` 已在原 production runner 部署。对外 HTTPS 的 `/healthz`、`/readyz`、`/login`、`/sspr` 均返回 200；就绪检查确认数据库、表结构和 worker 正常。管理员密码由生产宿主受限文件提供（0600），已通过公开 HTTPS 登录并产生成功审计。真实企业验收仍未完成。
...
- 生产入口的只读诊断确认 HTTPS 网关以 `10.106.1.119` 连接应用 Nginx。原实现按容器收到的代理地址限流，会让所有员工共享额度；当前 PR 限定仅信任该网关的转发地址，并由 Nginx 覆写应用读取的客户端地址。[网关诊断任务](https://github.com/liugang926/AD-Org-Sync/actions/runs/35825253657)。仍需部署后验证网关实际转发头、不同客户端额度隔离及伪造转发头被忽略。
...
- 检查系统定时任务，并在修正部署后验证网关转发头与限流行为；生产宿主连接配置和对外 HTTPS 已核验，不打印凭据。
