...
| FR-09～10 员工重置 | `tests/test_sspr.py`、`tests/test_browser.py`：未同步未绑定员工可重置、服务端身份、GUID/配置/过期/重放/CSRF 检查；#70 的浏览器回归验证自动免登、完整账号显示、失败重试和未验证访客不可见账号；跨线程占用同一账号锁时改密被拒且会话未消费，锁释放后可用原会话重试；隔离合成身份已在真实测试 AD 验证密码生效、LDAPS 登录、会话消费和无明文；生产审计已记录 `T0001919` 对应 userId 的验证及成功重置，AD 改密时间吻合且当前无本地绑定，员工本人操作与界面效果仍待确认 |
...
| AC-14 目标替换拒绝 | 服务端身份与输入篡改自动化；生产已有真实身份验证和重置成功审计 | 未在真实钉钉授权会话中演练目标替换拒绝 |
| AC-15 会话失效 | 过期、重放、配置与 GUID 变更自动化；生产已有真实身份验证和重置成功审计 | 未在生产对该会话演练重放、过期与配置变更 |
| AC-16 密码生效与无明文 | 合成身份、隔离 SQLite、真实 LDAPS 改密与重新登录；生产 `T0001919` 对应身份的重置审计成功，AD `pwdLastSet` 与审计时间吻合 | 尚未由员工本人确认工作台操作和新密码登录；不读取或索取密码 |
...
- AD 适配器和使用合成来源的隔离业务同步已用专用测试对象完成创建、重复执行、属性、移动、禁用与清理；真实钉钉来源已在隔离数据库中驱动一次性 AD 账号的指定人员属性更新，并在人工 OU 映射变化后驱动移动。生产 `T0001919` 对应身份已有成功改密审计和 AD 改密时间证据，仍需员工确认本人工作台操作、账号显示及新密码可登录；另需用可控钉钉测试人员完成真实来源的新建、实际部门变更与禁用验收。此前同步写入演练对 `T0001919` 的既有 AD 账号保持只读，当前线上该员工仍无绑定；禁止拿普通员工账号试验。
